...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function loadCfg() {
			...
			// 无 Ctrl/Alt/Meta 的裸键组合（如 Shift+A）与正常打字结构性冲突：
			// 聊天时输入大写字母既是快捷键又是打字，任何防误触窗口都救不回来——自动回退默认
			if (!merged.ctrl && !merged.alt && !merged.meta) return { ...CFG_DEFAULTS };
			return merged;
		}
		function saveCfg(c) {
		...
		function matchCombo(e, c) {
			// 防御纵深：无修饰键组合永不匹配（与打字结构性冲突，见 loadCfg 迁移说明）
		...
		function ShutupEntry({ session }) {
			...
			const sessionRef = useRef(session);   // latest-value ref：每次渲染后同步最新快照，异步路径读它拿到新鲜数据
			const bareContinueArmedRef = useRef(false);  // 摘取失败一次后，第二次确认才允许"不带上下文继续"
			useEffect(() => { sessionRef.current = session; });   // 无依赖 = 每次渲染都同步
			...
			// ---- 权威源（v1.3）：宿主日志里的最后一条 interrupted 消息（0.1.1 官方中断落盘） ----
			// 返回 { reasoning, text, usage }；null 表示未找到（超时/无中断记录）。调用带 4s 超时。
			const fetchAuthoritativeTail = async () => {
				try {
					const ctrl = new AbortController();
					const to = setTimeout(() => ctrl.abort(), 4000);
					try {
						const r = await fetch('/shutup/interrupted?sessionId=' + encodeURIComponent(sessionId), { signal: ctrl.signal });
						const j = await r.json();
						return j && j.ok === true && j.interrupted ? j.interrupted : null;
					} finally {
						clearTimeout(to);
					}
				} catch (e) {
			...
			// 继续原来的思考（方案 A v1.3）：权威源优先 → UI 快照兜底 → 裸 [继续] 警告。
			// 权威源：宿主读日志最后一条 interrupted 消息（完整无截断）；
			// 兜底：latest-value ref 读渲染快照（旧路径保留防宿主升级日志格式变化）；
			// 仍摘不到才走"二次确认"退化为裸 [继续]。回填投递到宿主（先于会话组装）。
			const continueThinking = async () => {
				const face = sessionFace();
				let tail = null;
				// 1) 权威源（中断落盘后内容完整、无截断；官方 append 是同步的，一次尝试即命中）
				const authoritative = await fetchAuthoritativeTail();
				if (authoritative && (authoritative.reasoning?.length > 0 || authoritative.text?.length > 0)) {
					tail = {
						reasoning: authoritative.reasoning ?? '',
						output: authoritative.text ?? '',
					};
					try { console.log('[dsh-shutup] backfill source: authoritative'); } catch (e) {}
				}
				// 2) 兜底：UI 快照（权威源失败时重试 6×500ms）
				if (tail === null) {
					for (let i = 0; i < 6 && tail === null; i++) {
						if (i > 0) await new Promise((resolve) => setTimeout(resolve, 500));
						tail = extractChainTail(sessionRef.current);
					}
				}
				try { console.log('[dsh-shutup] backfill tail:', tail === null ? 'null' : ('命中（reasoning ' + tail.reasoning.length + ' 字 / output ' + tail.output.length + ' 字）')); } catch (e) {}
				const shortMsg = '[继续] 继续你刚才的思考与工作。';
				if (tail === null) {
					// 摘不到上下文时绝不静默发裸 [继续]（会把思考链丢干净）：
					// 第一次 → 弹窗重开 + 警告（可能尚未落定，稍等再试）；
					// 第二次确认 → 用户明示接受不带上下文继续。
					if (!bareContinueArmedRef.current) {
						bareContinueArmedRef.current = true;
						setErrMsg('未取到被打断回合的思考上下文（可能尚未落定）。再点一次「继续」将不带上下文直接继续；或关闭后稍等几秒再按快捷键重试。');
						setOpen(true);
						return;
					}
					bareContinueArmedRef.current = false;
		...
		function SettingsSection() {
			...
			useEffect(() => {
				...
				const onKey = (e) => {
					...
					// 裸键组合（无 Ctrl/Alt/Meta）与正常打字结构性冲突，拒绝录制（保持录制态，可继续按新组合）
			...
			return h('div', { className: 'dsup-gen', children: [
				...
				h('div', { className: 'dsup-note', children: '模型思考/输出时按下快捷键：弹出弹窗（未输入不暂停）；开始输入才暂停当前生成；输入内容需二次确认后载入纠偏；空输入确认或取消 = 继续原来的思考。其他时间快捷键自动禁用。快捷键必须包含 Ctrl/Alt/Meta 至少一个（纯 Shift+字母与打字冲突，录制时会拒绝）。中文输入法激活时也能正常触发（按物理键匹配）；仅当 1.5s 内刚在输入框敲过字时才当作普通输入，其余情况一律触发。' }),
