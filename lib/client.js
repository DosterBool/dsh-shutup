	...
	factory: (require) => {
		...
		function ShutupEntry({ session }) {
			...
			const continueThinking = async () => {
				const face = sessionFace();
				let tail = null;
				// SessionFace 没有 getSnapshot（已核实官方类型：SessionBinding = { sessionId, session, ctx }）——
				// 旧实现的 getSnapshot 分支是死代码。改为 latest-value ref：每次重试都读当下渲染的最新快照，
				// 被打断回合一旦落定（重渲染后）必然命中，不再是单次陈旧读取。
