...
const name = 'shutup'
/** webServer：/shutup 前缀路由；systemPrompt：一次性上下文回填；sessionPersistence：v1.3 权威源读中断消息 */
const inject = ['webServer', 'systemPrompt', 'sessionPersistence']
...
function apply(ctx, config) {
  ...
  async function readLastInterrupted(sessionId) {
    try {
      ...
      if (!persistence || typeof persistence.readFrom !== 'function') return { __err: 'sessionPersistence unavailable' }
      const snap = await persistence.readFrom(sessionId, 0, AbortSignal.timeout(8000))
      const events = snap && Array.isArray(snap.events) ? snap.events : null
      if (!events) return { __err: 'no events' }
      ...
      return { __err: 'no interrupted message in events, lastTypes: ' + events.slice(-3).map((x) => x.type).join(',') }
    } catch (e) {
      return { __err: 'load threw: ' + String(e).slice(0, 200) }
    }
  }
